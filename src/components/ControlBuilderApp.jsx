...
import { Plus, Send, Upload, Download, RefreshCw, AlertTriangle } from 'lucide-react';
...
export function ControlBuilderApp() {
  ...
  const handleDuplicateControl = (control) => {
    const newId = `${control.id}-COPY`;
    const duplicate = {
      ...control,
      id: newId,
      scoped_id: control.source_file ? `${control.source_file}:${newId}` : newId,
      anchor: `${newId.toLowerCase()}-${control.name?.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/-+$/, '') || ''}`,
    };
    setControls([...controls, duplicate]);
    setSelectedControl(duplicate);
  };

  const handleDeleteControl = (control) => {
    const key = control.scoped_id || control.id;
    setControls(controls.filter((c) => (c.scoped_id || c.id) !== key));
    if (selectedControl && (selectedControl.scoped_id || selectedControl.id) === key) {
